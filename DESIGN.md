...
6. **Dependency Boundary Rule.** Modules are classified into three tiers:

    | Tier | Examples | May require |
    |------|----------|-------------|
    | **Coordinator** | `init.lua`, `stream.lua`, `ui/init.lua`, `commands/init.lua` | Anything |
    | **Utility** | `util/tokens.lua`, `util/strings.lua` | Only stdlib / vim APIs |
    | **Boundary** | `providers/*`, `models.lua`, `context/*` | Utilities only |

    **Coordinator modules** orchestrate the plugin. They may call
    `config.get()`, `require` other internal modules, and wire dependencies
    together.

    **Utility modules** are pure-function libraries with no internal state and
    no `require` calls into the plugin. They depend only on Lua stdlib and
    `vim.*` APIs. Anyone may require them — they are leaf nodes in the
    dependency graph.

    **Boundary modules** are self-contained units that could work outside this
    plugin. They receive everything they need as function arguments and must
    not `require("ai-chat.config")`, `require("ai-chat.models")`, or any
    other coordinator-owned module directly. They *may* require utility
    modules (e.g., `require("ai-chat.util.tokens")`).
